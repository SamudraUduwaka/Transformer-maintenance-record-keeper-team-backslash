...
import {
  ...
  DialogTitle, Divider, Drawer, FormControl, IconButton, List, ListItem, ListItemButton,
  ListItemIcon, ListItemText, MenuItem, Paper, Select, Stack, ThemeProvider, Toolbar,
...
function StatPill({ top, bottom }: { top: string | number; bottom: string }) {
  return (
    <Box sx={{ px: 1.5, py: 1, borderRadius: 3, bgcolor: "#EEF0F6", minWidth: 108, display: "inline-flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
...
export default function InspectionDetails() {
  ...
  React.useEffect(() => {
    ...
    const t = setTimeout(() => {
      navigate(`/${transformerNo}/${inspectionNo}/comparison`, { replace: true });
    }, 650);
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <AppBar position="fixed" color="inherit" elevation={0}
        ...
        <Toolbar sx={{ minHeight: 64 }}>
          <Stack direction="row" spacing={1.25} alignItems="center">
            ...
            <Typography variant="h6" sx={{ fontWeight: 800 }}>Transformer</Typography>
          </Stack>
          <Box sx={{ flexGrow: 1 }} />
          <Tooltip title="Notifications">
            <IconButton><Badge color="secondary" variant="dot"><NotificationsIcon /></Badge></IconButton>
          </Tooltip>
          ...
          <Stack spacing={2}>
              ...
              <Stack direction="row" alignItems="stretch" sx={{ width: "100%" }}>
                ...
                <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                  <Stack direction="row" alignItems="center" spacing={1.25}>
                    <Box sx={{ width: 28, height: 28, borderRadius: "50%", bgcolor: "#1F1C4F", display: "grid", placeItems: "center" }}>
                      <Box sx={{ width: 10, height: 10, bgcolor: "#FFFFFF", borderRadius: "50%" }} />
                    </Box>
                    <Typography variant="h6" fontWeight={800}>{inspectionNo}</Typography>
                ...
                <Stack direction="column" alignItems="flex-end" justifyContent="space-between" sx={{ alignSelf: "stretch", minWidth: 350, py: 0.5 }}>
                  <Box sx={{ width: "100%", display: "flex", justifyContent: "flex-end" }}>
                    <Chip label="In progress" color="success" variant="outlined"
            ...
            {isUploading ? (
              ...
              <Paper elevation={3} sx={{ p: 3, borderRadius: 2 }}>
                ...
                <Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 3, p: { xs: 3, sm: 6 }, textAlign: "center", bgcolor: "white" }}>
                  ...
                  <Box sx={{ mt: 3, mx: "auto", width: { xs: "100%", sm: "70%" } }}>
                    <Box sx={{ height: 8, borderRadius: 999, bgcolor: "#E5E7EB" }}>
                      <Box sx={{ height: "100%", width: `${progress}%`, bgcolor: "primary.main", borderRadius: 999, transition: "width 200ms linear" }} />
                    </Box>
                    <Box sx={{ mt: 0.5, textAlign: "right", color: "text.secondary", fontSize: 12 }}>{progress}%</Box>
                  </Box>
                  <Button onClick={cancelUploadProgress} sx={{ mt: 3, borderRadius: 999 }}>
                    Cancel
                  </Button>
                </Box>
              </Paper>
                ...
                <Paper elevation={3} sx={{ p: 2.25, borderRadius: 1 }}>
                  <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
                    <Stack direction="row" spacing={1} alignItems="center">
                      ...
                      <Typography variant="subtitle1" fontWeight={800}>Baseline Images</Typography>
                    </Stack>
                    <Chip size="small" label={`Selected Weather: ${weather}`} sx={{ bgcolor: "#EEF0F6", fontWeight: 600 }} />
                  </Stack>
                  ...
                  <Stack direction="row" spacing={2} justifyContent="space-between" sx={{ width: "100%" }}>
                    {(["Sunny", "Cloudy", "Rainy"] as const).map((w) => {
                      ...
                      const Icon = w === "Sunny" ? SunnyIcon : w === "Cloudy" ? CloudIcon : RainIcon;
                      ...
                      return (
                        <Paper
                          ...
                          variant="outlined"
                          sx={{
                            width: 240, height: 240, flex: 1, p: 1.5, borderRadius: 1.5,
                            borderStyle: available ? "solid" : "dashed", display: "flex", flexDirection: "column",
                          }}
                        >
                          <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
                            <Stack direction="row" spacing={1} alignItems="center">
                            ...
                            <Chip size="small" label={available ? "Available" : "Not set"} color={available ? "success" : "default"} variant="outlined" />
                          </Stack>

                          <Box
                            sx={{
                              flexGrow: 1, borderRadius: 1.5, overflow: "hidden", bgcolor: "#F3F4F6",
                              display: "flex", alignItems: "center", justifyContent: "center", mb: 1, height: 140,
                            }}
                          >
                            {available ? (
                              <img src={item!.url} alt={`${w} baseline`} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                            ) : (
                          ...
                          <Stack direction="row" spacing={1} justifyContent="flex-end">
                            {!available ? (
                              <Button size="small" variant="contained" onClick={() => openUploadFor(w)}>Upload</Button>
                            ) : (
                              <>
                                <Button size="small" variant="contained" onClick={() => viewBaseline(w)}>View</Button>
                                <Button size="small" color="error" onClick={() => deleteBaseline(w)}>Delete</Button>
                              </>
                            )}
                          </Stack>
                        </Paper>
                ...
                <Stack direction={{ xs: "column", lg: "row" }} spacing={2} alignItems="stretch">
                  ...
                  <Paper elevation={3} sx={{ p: 2.5, borderRadius: 2, flex: 1 }}>
                    <Stack direction="row" alignItems="center" spacing={1}>
                      <Typography variant="subtitle1" fontWeight={700}>Thermal Image</Typography>
                    ...
                    <Box sx={{ mt: 3 }}>
                      <Typography variant="caption" color="text.secondary">Weather Condition</Typography>
                    ...
                    <Button
                      fullWidth variant="contained" sx={{ mt: 3, borderRadius: 999, py: 1.1, fontWeight: 700 }}
      ...
      <Dialog open={uploadOpen} onClose={handleCloseUpload} fullWidth maxWidth="sm"
        PaperProps={{ sx: { borderRadius: 1.5, boxShadow: "0 10px 36px rgba(15,23,42,0.22)" } }}>
        <DialogTitle sx={{ px: 3, py: 1.75 }}>
          <Typography variant="h6" fontWeight={700}>Upload Thermal Image</Typography>
        </DialogTitle>
        <DialogContent dividers sx={{ px: 3, pt: 1.25, pb: 1.5, bgcolor: "#FBFBFE", minHeight: 300 }}>
          <Box sx={{ my: 2, p: 2, border: "1px dashed", borderColor: "divider", borderRadius: 2, textAlign: "center", bgcolor: "white" }}>
            {preview ? (
              <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 1.25 }}>
                ...
                <Button size="medium" onClick={() => { setFile(null); if (preview) URL.revokeObjectURL(preview); setPreview(null); }}
              ...
              <>
                <Typography variant="body1" gutterBottom fontWeight={600}>Drag & drop an image here</Typography>
                <Typography variant="body2" color="text.secondary">or click the button below to browse your device</Typography>
        ...
        <DialogActions sx={{ px: 3, py: 1.25 }}>
          <Button onClick={handleCloseUpload} size="medium" sx={{ px: 2.25, py: 0.6, fontSize: 14 }}>Cancel</Button>
          <Button variant="contained" size="medium" onClick={handleConfirmUpload} disabled={!file}
      ...
      <Dialog open={Boolean(manageWeather)} onClose={closeUploadFor} fullWidth maxWidth="sm">
        ...
        <DialogContent dividers sx={{ bgcolor: "#FBFBFE" }}>
          <Box sx={{ my: 1, p: 2, border: "1px dashed", borderColor: "divider", borderRadius: 2, textAlign: "center", bgcolor: "white" }}>
            {managePreview ? (
        ...
        <DialogActions sx={{ px: 3, py: 1.25 }}>
          ...
          <Button variant="contained" disabled={!manageFile && !managePreview} onClick={confirmBaselineUpload}>Save</Button>
        </DialogActions>
      </Dialog>

      {/* ===== Baseline Viewer ===== */}
      <Dialog open={viewer.open} onClose={() => setViewer({ open: false })} maxWidth="md" fullWidth>
        <DialogTitle><Typography fontWeight={700}>View {viewer.weather} Baseline</Typography></DialogTitle>
        <DialogContent dividers>
          {viewer.url ? <img src={viewer.url} alt="baseline" style={{ width: "100%", height: "auto", display: "block", borderRadius: 8 }} /> : <Typography>No image</Typography>}
        </DialogContent>
        <DialogActions><Button onClick={() => setViewer({ open: false })}>Close</Button></DialogActions>
      </Dialog>
    </ThemeProvider>
  );
}
