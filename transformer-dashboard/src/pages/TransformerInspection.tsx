...
//   const canConfirmAdd = branch.trim() && tNo.trim() && date && time && inspector.trim();
//   const canConfirmEdit = editBranch.trim() && editTNo.trim() && editDate && editTime && editInspector.trim();
...
//         // transformer
//         const t = await http<TransformerDTO>(`/transformers/${encodeURIComponent(transformerNo)}`);
//         setTransformer(t);

//         // inspections (filter by transformerNo)
//         const all = await http<InspectionDTO[]>(`/inspections`);
//         const mine = all.filter((i) => i.transformerNo === transformerNo).sort((a, b) => b.inspectionId - a.inspectionId);
...
//         inspectionTime,
//       };
//       const created = await http<InspectionDTO>("/inspections", { method: "POST", json: payload });
...
//   // last inspected (derived from rows)
//   const lastInspectedAt = React.useMemo(() => rows[0]?.inspectedDate ?? "-", [rows]);
...
//                                 onClick={() =>
//                                   navigate(
//                                     `/${encodeURIComponent(row.transformerNo)}/${encodeURIComponent(row.inspectionNo)}`
//                                   )
//                                 }
//                               >
//                                 View
...
//       <Dialog open={deleteOpen} onClose={handleCloseDelete} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: 2 } }}>
//         <DialogTitle sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
//           <Typography fontWeight={700} fontSize="1.25rem">
//             Confirm Delete
//           </Typography>
