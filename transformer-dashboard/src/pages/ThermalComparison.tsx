...
export default function ThermalComparison() {
  ...
  const [baselineUrl, setBaselineUrl] = React.useState<string | null>(null);
  const [currentUrl, setCurrentUrl] = React.useState<string | null>(null);
  const [loadingImgs, setLoadingImgs] = React.useState(true);
  const [imgErr, setImgErr] = React.useState<string | null>(null);

  // optional: transform Cloudinary URLs for fast delivery
  function transformIfCloudinary(url?: string | null, width = 1400) {
    if (!url) return "";
    return url.includes("res.cloudinary.com")
      ? url.replace("/upload/", `/upload/f_auto,q_auto,w_${width},c_fit/`)
      : url;
  }

  //const inspectionId = Number(inspectionNo); // adjust if the route param isn't the DB PK
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Pull all images (your controller doesn't filter yet)
        const res = await fetch(`${API_BASE_URL}/images`);
        if (!res.ok) throw new Error("Failed to load images");
        const all: ImageDTO[] = await res.json();

        const items = all.filter((i) => i.inspectionId === inspectionId);

        // choose latest by createdAt (fallback to updatedAt)
        const ts = (i: ImageDTO) =>
          new Date(i.createdAt ?? i.updatedAt ?? 0).getTime();

        const latestBaseline = items
          .filter((i) => i.type?.toLowerCase() === "baseline")
          .sort((a, b) => ts(a) - ts(b))
          .at(-1);

        const latestThermal = items
          .filter((i) => i.type?.toLowerCase() === "thermal")
          .sort((a, b) => ts(a) - ts(b))
          .at(-1);

          console.log({items, latestBaseline, latestThermal});

        if (!cancelled) {
          setBaselineUrl(latestBaseline?.imageUrl ?? null);
          setCurrentUrl(latestThermal?.imageUrl ?? null);
        }
      } catch (e: any) {
        if (!cancelled) setImgErr(e?.message ?? "Error loading images");
      } finally {
...
//const { state } = useLocation() as { state?: { weather?: "Sunny"|"Cloudy"|"Rainy" } };
//const preferredWeather = state?.weather;
// …filter baselines by weatherCondition === preferredWeather before picking the latest
  ...
  const CURRENT_URL =
    "https://res.cloudinary.com/djgapcqtj/image/upload/v1755962475/y2gcbm8qxlwq6d6gb00t.webp";
          ...
          <Stack spacing={2}>
            ...
            >
              ...
              <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
                ...
                >
                  <img
                    //src={transformIfCloudinary(baselineUrl) || ""}
                    src={BASELINE_URL}
                    alt="Baseline"
                    style={{
                      position: "absolute",
                ...
                >
                  <img
                    //src={transformIfCloudinary(currentUrl) || ""}
                    src={CURRENT_URL}
                    alt="Current"
                    style={{
                      position: "absolute",
