...
export default function Dashboard() {
  ...
  const confirmAdd = async () => {
    ...
    try {
      ...
      showSnackbar(err instanceof Error ? err.message : "Failed to add transformer", "error");
    }
  };
  ...
  const handleDeleteTransformer = async (transformerNo: string) => {
    if (!window.confirm("Are you sure you want to delete this transformer?")) return;
    
    try {
      await transformersAPI.delete(transformerNo);
      setRows(rows.filter(t => t.transformerNo !== transformerNo));
                            ...
                            <TableRow key={row.transformerNo} hover>
                              ...
                              <TableCell align="right">
                                <Stack direction="row" spacing={1} justifyContent="flex-end">
                                  ...
                                  <IconButton onClick={() => handleDeleteTransformer(row.transformerNo)}>
                                    <MoreVertIcon />
                                  </IconButton>
                                </Stack>
                              </TableCell>
      ...
      <Dialog open={openAdd} onClose={closeAddDialog} fullWidth maxWidth="sm">
        ...
        <DialogContent sx={{ pt: 1 }}>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <FormControl fullWidth size="small">
              <InputLabel id="region-label">Region</InputLabel>
              <Select labelId="region-label" label="Region" value={form.region} 
                onChange={updateField("region")} error={!!errors.region}>
                {REGIONS.map((r) => <MenuItem key={r} value={r}>{r}</MenuItem>)}
              </Select>
            </FormControl>
            <TextField size="small" label="Transformer No" value={form.transformerNo} 
              onChange={updateField("transformerNo")} error={!!errors.transformerNo} 
              helperText={errors.transformerNo ? "Required" : ""} fullWidth />
            <TextField size="small" label="Pole No" value={form.poleNo} 
              onChange={updateField("poleNo")} error={!!errors.poleNo} 
              helperText={errors.poleNo ? "Required" : ""} fullWidth />
            <FormControl fullWidth size="small">
              <InputLabel id="type-label">Type</InputLabel>
              <Select labelId="type-label" label="Type" value={form.type} 
                onChange={updateField("type")} error={!!errors.type}>
                {TYPES.map((t) => <MenuItem key={t} value={t}>{t}</MenuItem>)}
              </Select>
            </FormControl>
            <TextField size="small" label="Location Details" value={form.location} 
              onChange={updateField("location")} fullWidth multiline minRows={3} />
          </Stack>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button variant="contained" onClick={confirmAdd}>Confirm</Button>
