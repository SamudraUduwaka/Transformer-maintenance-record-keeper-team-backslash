...
def main():
    ...
    ap.add_argument("--nms_mask_iou", type=float, default=0.60,
                    help="IoU threshold to suppress yellow classes that overlap red ones (only if --suppress_yellow_with_red)")
    ap.add_argument("--suppress_yellow_with_red", action="store_true",
                    help="If set, remove yellow class detections whose polygon bbox overlaps a red class bbox >= --nms_mask_iou")
    ...
    with open(jsonl_path, "w", encoding="utf-8") as f:
        for p in paths:
            ...
            # Optional suppression: if requested, remove yellow classes (0,2,4) heavily overlapping red classes (1,3)
            if args.suppress_yellow_with_red and len(classes) > 0:
                # Build simple bounding boxes from polygons for fast IoU
                bboxes = []  # (x1,y1,x2,y2)
                ...
                red_idxs = [i for i,c in enumerate(classes) if c in (1,3)]
                yellow_idxs = [i for i,c in enumerate(classes) if c in (0,2,4)]
                ...
                for yi in yellow_idxs:
                    for ri in red_idxs:
                        if not keep_mask[yi]:
                            break
                        if iou(bboxes[yi], bboxes[ri]) >= args.nms_mask_iou:
                            keep_mask[yi] = False
                if not all(keep_mask):
                    classes = [c for k,c in zip(keep_mask, classes) if k]
                    confs   = [c for k,c in zip(keep_mask, confs)   if k]
                    masks_xy= [m for k,m in zip(keep_mask, masks_xy)if k]
